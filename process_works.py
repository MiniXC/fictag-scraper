...
for _, row in tqdm(fandom_df.iterrows(), total=len(fandom_df)):
    ...
    for work_path in tqdm(work_glob, total=len(work_glob), desc=f'get unknown tags for {fandom}'):
        if (processed_works_path / work_path.name.replace(".csv", ".csv.gz")).exists() and not args.overwrite:
            continue
        unknown_tags = {}
        work_add_general_tag_ids = []
        work_add_fandom_tag_ids = []
        work_df = pd.read_csv(work_path)
        for work_idx, work_row in tqdm(work_df.iterrows(), total=len(work_df), desc=f'get unknown tags for {fandom}'):
            # eval
            ...
            for tag in tags_to_remove:
                try:
                    ...
                    pass
            if len(tags) == 0 and len(add_general_tag_ids) == 0 and len(add_fandom_tag_ids) == 0:
